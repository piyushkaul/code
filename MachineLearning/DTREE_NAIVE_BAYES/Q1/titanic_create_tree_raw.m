...
% Apr 2016
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function nodeout = titanic_create_tree_raw(nodein, traindata, idx, depth)
global max_node_id; 
global depth_ids;
...
if numel(unique(traindata(:,1))) == 1
    ...
    [attribute_id, partitions, medval] = get_best_atribute(traindata, 'raw');
end    
...
nodeout.attribute_threshold_history = nodein.attribute_threshold_history;
if attribute_id ~= 0
nodeout.attribute_use_history(attribute_id)  = nodeout.attribute_use_history(attribute_id)  +  1;
nodeout.attribute_threshold_history(attribute_id, nodeout.attribute_use_history(attribute_id)+1)= medval;
end
nodeout.depth = depth;
...
if numel(partitions) == 1
    ...
    for ix = 1: numel(partitions);
         ...
         nodeout.children{ix} = titanic_create_tree_raw(nodeout, partition, ix, depth+1);
    end 
end
